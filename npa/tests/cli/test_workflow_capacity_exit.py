...
def temporary_preflight():
    cause = TemporarilyUnavailableAcceleratorError("One node must become free")
    wrapper = ExecutionPreflightError("gpu", "capacity unavailable", status="unknown")
    wrapper.__cause__ = cause
    return wrapper


def test_typed_preflight_cause_survives_execution_wrapper():
    assert _submit_failure_code(temporary_preflight()) == 75


@pytest.mark.parametrize("attempted,expected", [(False, 75), (True, 1), (None, 1)])
def test_final_preflight_race_retries_only_with_no_provider_launch(attempted, expected):
